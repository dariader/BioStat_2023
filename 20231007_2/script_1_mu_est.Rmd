...
```{r conclusions}
values
true_mean <- sum(values*Pr)
true_mean
variance <- sum((df_sample_mean$mean_Hg_upd-true_mean)^2)/nrow(df_sample_mean)
variance
SD <- sqrt(variance)
SD # в четыре раза увеличили выборку, в корень из четырех уменьшилась ошибка
